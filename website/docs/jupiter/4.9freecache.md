...
func NewInstance() *Instance {
	return &Instance{
		localCache: xfreecache.NewLocalCache[string, Student](xfreecache.DefaultConfig()),
	}
}
```
- 默认配置详情：缓存容量：`256MB`（缓存value需要小于256KB）   失效时间：`2分钟`  是否开启metric上报：`是`
- 初始化多个默认配置时，内部只会初始化一个缓存容量魏256MB的缓存实例。无需担心初始化多个实例后的缓存消耗
...
func NewInstance() *Instance {
	return &Instance{
		localCache: xfreecache.NewLocalCache[int32, []int32](xfreecache.DefaultConfig().SetExpire(1*time.Minute)),
	}
}
