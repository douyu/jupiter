...
## 4.9.1 简介
pkg/cache/xfreecache 包支持多个本地缓存库，并支持metric上报
...
[jupiter.cache]
    ...
    size = "256MB" 
    # golang-lru 本地缓存总容量 默认200000
    # 
    sizeLru = 200000
[jupiter.cache.student]
    expire = "2m" # 本地缓存失效时间
    disableMetric = false # 是否禁用metric上报 false 开启  ture 关闭  默认开启上报
    cacheType = "freecache" # 选择需要使用的本地缓存库，默认使用freecache  可选：freecache、lru
```

#### 支持的本地缓存库
|    cacheType字段枚举     |     描述     |                               库地址                                |
|:---------:|:----------:|:----------------------------------------------------------------:|
| freecache | freecache库 | [github.com/coocood/freecache](https://github.com/coocood/freecache)   |
| lru | golang-lru库 | [github.com/hnlq715/golang-lru](https://github.com/hnlq715/golang-lru)   |
...
## 4.9.3 不使用泛型的用法（返回字节数组，需要自己实现序列化）
### 1 初始化实例
#### 1) 使用默认的初始化方式
- 初始化方式
```go
