...
import (
	...
	"github.com/douyu/jupiter/pkg/util/xfieldmask"
...
func TestNestedFieldMask(t *testing.T) {
	...
	t.Run("test Filter method", func(t *testing.T) {
		msg := &helloworldv1.SayHelloResponse{
			Error: 0,
			Msg:   "请求正常",
			Data: &helloworldv1.SayHelloResponse_Data{
				Name:      "foo",
				AgeNumber: 18,
				Sex:       helloworldv1.Sex_SEX_MALE,
				Metadata: map[string]string{
					"Bar": "bar",
				},
			},
		}
		nfm := xfieldmask.New([]string{"data", "data.name", "data.age_number", "data.metadata"})
		nfm.Filter(msg)
		...
		assert.Equal(t, helloworldv1.Sex_SEX_UNSPECIFIED, msg.GetData().GetSex())
	...
	t.Run("test Prune method", func(t *testing.T) {
		...
		assert.Equal(t, helloworldv1.Sex_SEX_MALE, msg.GetData().GetSex())
