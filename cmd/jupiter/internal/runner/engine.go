...
func (e *Engine) runBin() error {
	...
	bin := e.config.Build.Bin + " " + e.config.Build.Args
	e.runnerLog("running (%s)...", bin)
	cmd, stdout, stderr, err := e.startCmd(bin)
	if err != nil {
		return err
	}
	e.withLock(func() {
...
func (e *Engine) cleanup() {
	...
	e.withLock(func() {
		for i := 0; i < int(e.watchers); i++ {
			e.mainDebug("send watcherStopCh")
